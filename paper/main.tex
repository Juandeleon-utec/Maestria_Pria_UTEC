...
Sobre la viabilidad económica del cambio de paradigma en este caso: asumiendo un costo conservador de parada no planificada de USD 2.000/hora y una reducción del 30\% en eventos de falla --- cifra atribuida a \citet{abdulkareem2025} en la formulación original de esta propuesta, aunque no fue posible verificar de forma independiente, a partir del artículo primario, que esa cifra corresponda exactamente a lo reportado por los autores ---, el retorno sobre la inversión del sistema propuesto se estimaría en menos de seis meses. Esta proyección debe leerse como estrictamente ilustrativa y no como un resultado validado: además de la incertidumbre sobre la fuente de la cifra del 30\%, la comparación involucra condiciones de motor, carga y régimen de fallas que no necesariamente coinciden con los de la planta objeto de este trabajo. El objetivo de incluirla aquí es mostrar la forma que tomará la evaluación económica del sistema, no anticipar su resultado; una estimación de ROI verificable requiere, como mínimo, la frecuencia histórica de fallas de la planta, la duración promedio de sus paradas y el costo total del sistema propuesto, ninguno de los cuales está determinado en esta etapa.
...
El Trabajo Final de Especialización previo del autor \citep{deleonsum2021} constituye el punto de partida empírico directo de esta investigación. Aquel trabajo, desarrollado durante el período de aislamiento por la pandemia de COVID-19 y sin acceso a laboratorio universitario o a planta industrial, validó una arquitectura basada en redes LSTM (\textit{Long Short-Term Memory}) para detección de fallas por vibración sobre dos prototipos básicos --- uno conectado a un motor monofásico y otro a un motor trifásico ---, alcanzando una precisión superior al 90\% (95,30\% según lo documentado en el artículo derivado, en preparación para IEEE URUCON 2026) en esas condiciones acotadas, correspondiente a un nivel de madurez tecnológica TRL 4. Este resultado confirma la viabilidad conceptual del enfoque general --- detección de fallas por vibración mediante aprendizaje profundo ---, pero, precisamente por haberse obtenido sobre dos prototipos básicos sin validación en planta, no resuelve la pregunta de si el sistema es operacionalmente confiable en un entorno industrial real.
...
\textbf{Una limitación de instrumentación.} El sensor utilizado en el trabajo de 2021 (MPU-6050, bus I\textsuperscript{2}C) conlleva un riesgo conocido de susceptibilidad a interferencia electromagnética (EMI), asociado al diseño del propio bus I\textsuperscript{2}C frente a alternativas como SPI --- una limitación de instrumentación documentada de forma general en la literatura de adquisición de señal industrial, no verificada específicamente en el trabajo de 2021, que no incluyó pruebas de robustez frente a EMI: los dos prototipos se construyeron y probaron en condiciones básicas, sin acceso a laboratorio ni a un entorno con motores de gran potencia o variadores de frecuencia. Este riesgo no verificado, sumado a la ausencia de validación en condiciones industriales reales durante ese trabajo, motiva la migración a un sensor SPI de menor ruido en la presente propuesta, sujeta a la verificación experimental correspondiente en la Fase 1 de la metodología. Esta situación es, en todo caso, metodológicamente relevante para la presente propuesta: ilustra que la validación sobre prototipos básicos, aunque necesaria como primer paso, es insuficiente como criterio único de madurez tecnológica en este dominio.

Ambas limitaciones --- algorítmica y de instrumentación --- definen requisitos técnicos concretos que cualquier arquitectura sucesora debe satisfacer: capacidad de modelar dependencias temporales largas con mayor interpretabilidad que una LSTM, y un canal de adquisición de señal robusto a interferencia electromagnética en condiciones de planta real. La revisión de literatura desarrollada en la Sección~\ref{sec:estado-del-arte} examina en qué medida las arquitecturas convolucionales y de atención temporal (TCN, TFT) y los enfoques basados en grafos (GNN) abordan estos requisitos --- individualmente, en trabajos existentes --- y en qué medida ninguno de ellos ha sido validado de forma integrada bajo condiciones operacionales equivalentes a TRL 7. Esta constatación --- que el problema no es la inexistencia de soluciones parciales, sino la ausencia de su integración y validación conjunta en planta real --- es la que motiva directamente las preguntas de investigación que estructuran esta propuesta.

\subsection{Preguntas de investigación}
\begin{itemize}
    \item[] \textbf{PI-1:} ¿En qué medida una arquitectura TCN cuantizada, evaluada sobre el mismo conjunto de señales, supera el desempeño de detección de una LSTM (línea de base), y es dicha arquitectura viable dentro de las restricciones de memoria y cómputo de un nodo embebido ESP32-S3?
    \item[] \textbf{PI-2:} ¿En qué medida la incorporación de temperatura como variable complementaria a la vibración, mediante una arquitectura TFT, mejora el pronóstico del indicador de salud del motor a 24 horas respecto a un modelo entrenado exclusivamente con vibración?
    \item[] \textbf{PI-3:} ¿Es posible reducir significativamente la tasa de falsos positivos asociada a la contaminación cruzada de vibraciones entre activos adyacentes mediante una GNN cuyos nodos representen motores físicamente distintos, en comparación con un modelo individual por motor y con un modelo multivariable sin grafo?
\end{itemize}
...
Desarrollar, implementar y validar operacionalmente, en una planta agroindustrial uruguaya con más de 100 motores en operación continua, un framework distribuido de código abierto para el monitoreo predictivo de motores de inducción trifásicos, mediante una arquitectura jerárquica de inteligencia artificial que integre detección de anomalías en el borde a partir de datos multimodales de vibración y temperatura, pronóstico de vida útil remanente y modelado del acoplamiento vibracional entre activos adyacentes, con el propósito de avanzar el nivel de madurez tecnológica del sistema desde TRL 4 hacia una demostración operacional compatible con TRL 7, empleando procedimientos de medición y evaluación vibracional compatibles con la familia de normas ISO 20816, en la medida en que resulten aplicables.
...
\begin{description}
    ...
    \item[OE3.] Desplegar una red de nodos sensores en la planta agroindustrial y validar operacionalmente una arquitectura GNN capaz de modelar la topología de acoplamiento vibracional entre motores adyacentes, midiendo la reducción efectiva de falsos positivos respecto a un sistema sin modelado de grafos, y evaluando el sistema completo bajo procedimientos de medición y evaluación vibracional compatibles con la familia de normas ISO 20816, en la medida en que resulten aplicables.
\end{description}
...
Las redes LSTM se consolidaron durante la última década como la arquitectura por defecto para el diagnóstico de fallas basado en series temporales de vibración, y con razón: su capacidad de retener información a través de puertas de memoria explícitas --- a diferencia de las RNN simples --- las hace considerablemente más robustas frente a dependencias de mediano plazo. Esta consolidación se refleja en la literatura reciente de mantenimiento predictivo en motores de inducción: modelos híbridos que combinan capas convolucionales con unidades recurrentes (CNN-GRU, CNN-LSTM) han reportado precisiones de 92,57\% y 92,27\% respectivamente en la clasificación de fallas en motores de jaula de ardilla, sobre un conjunto de un millón de muestras balanceadas \citep{plosone2025}, y enfoques más clásicos basados en k-NN y ANN han alcanzado cifras del orden del 89--91\% \citep{abdulkareem2025}. Estas cifras no son directamente comparables entre sí --- provienen de datasets, variables de entrada, tipos de motor y condiciones de falla distintos, y no deben leerse como evidencia de un techo de desempeño compartido por el dominio --- pero sí ilustran, de forma cualitativa, que arquitecturas de naturaleza distinta alcanzan resultados en un rango similar, lo cual no permite por sí solo determinar si el factor limitante es la arquitectura de clasificación o las condiciones aguas arriba (calidad de la señal, representatividad del conjunto de entrenamiento).
...
Con TCN resolviendo la limitación algorítmica y TFT aportando pronóstico probabilístico con interpretabilidad parcial, queda un tercer problema sin resolver, que ninguna de las dos arquitecturas anteriores aborda porque ninguna de las dos fue diseñada para hacerlo: \textbf{ambas modelan cada activo de forma aislada}. Ni TCN ni TFT tienen, por construcción, ningún mecanismo para representar que la vibración de un motor puede estar correlacionada --- mecánicamente, no solo estadísticamente --- con la vibración de un motor físicamente adyacente en la misma línea de producción. En una planta con más de 100 motores, esta omisión no es trivial: la hipótesis de trabajo de esta tesis es que la contaminación cruzada de vibraciones entre activos próximos podría contribuir a la generación de falsos positivos, en la medida en que un modelo univariado por activo no tiene, por diseño, información para distinguir esa contaminación de una falla real. Esta es una hipótesis a verificar experimentalmente --- comparando, como mínimo, un modelo individual por motor, un modelo multivariable sin grafo y la GNN propuesta ---, no un hallazgo ya demostrado en esta etapa.
...
Cabe señalar, para cerrar este apartado con la debida cautela científica, que esta observación se basa en el conjunto de trabajos efectivamente revisados y no constituye una afirmación de exhaustividad: \textbf{[Referencia requerida] una búsqueda sistemática adicional, con criterios de inclusión/exclusión explícitos y protocolo tipo PRISMA, sería necesaria antes de sostener esta observación como un hallazgo de revisión sistemática formal y no como un patrón observado en una muestra de literatura} --- \citet{li2022gnnbenchmark} aporta un anclaje parcial a esta observación, pero fue identificado mediante búsqueda temática, no mediante un protocolo sistemático diseñado específicamente para esta pregunta. Del mismo modo, conviene mencionar que existen intentos de despliegue de bajo costo en el borde con sensores IEPE \textbf{[Referencia requerida --- la cita usada en una versión anterior de este capítulo no pudo verificarse: el DOI correspondía a un artículo de un tema completamente distinto, y debe buscarse nuevamente en Scopus antes de citar]}, y \citet{researchgate2025} con un nodo ESP32 y acelerómetro ADXL345 mediante TinyML, alcanzando más de 92\% de precisión --- pero ninguno de ellos incorpora modelado de grafo alguno: se limitan a inferencia univariada por nodo, sin ningún mecanismo de razonamiento sobre topología de planta.
...
Sobre este último punto --- la validación TRL 6--7 de arquitecturas de aprendizaje profundo en mantenimiento predictivo industrial --- la revisión sistemática PRISMA 2020 de \citet{henaovilla2026}, que examina 89 estudios sobre inteligencia artificial para mantenimiento predictivo en sistemas de manufactura compleja, aporta la confirmación cuantitativa más sólida disponible para el argumento central de este capítulo: el 65,6\% de los estudios analizados emplea protocolos de validación débiles o no especificados (según la propia clasificación por niveles de rigor de validación de los autores), y ningún estudio del corpus alcanzó validación operacional en campo más allá de una partición temporal o cruzada entre dominios. \textbf{La equivalencia entre esa clasificación de rigor de validación y la escala TRL usada en esta tesis es una interpretación propia, no una afirmación textual de los autores}, quienes no clasifican explícitamente sus estudios por nivel TRL; esta correspondencia debe verificarse releyendo la metodología completa del artículo antes de sostenerla sin matices en la versión final. Con esa salvedad, la cifra sigue siendo la evidencia sistemática y cuantificada más sólida disponible --- ajena a la revisión temática realizada para este capítulo --- para sostener que la validación en condiciones operacionales reales es, en general, poco frecuente en esta literatura.
...
Esta conexión permite formular con precisión las preguntas de investigación que estructuran los objetivos específicos de este trabajo: si una TCN cuantizada, evaluada sobre las mismas señales que la línea de base, supera su desempeño dentro de las restricciones de un nodo embebido (PI-1); si la incorporación de temperatura mediante TFT mejora el pronóstico del indicador de salud a 24 horas frente a un modelo basado solo en vibración (PI-2); y si una GNN cuyos nodos representan motores físicamente distintos --- desplazando deliberadamente la granularidad identificada como ausente en la Sección~\ref{sec:soluciones-emergentes}, de intra-activo a inter-activo --- logra reducir la tasa de falsos positivos frente a alternativas sin modelado de grafo (PI-3). Ninguna de las tres preguntas es trivialmente respondible a partir de la literatura revisada, precisamente porque ninguno de los trabajos examinados formula el problema en estos términos.
...
\item Validación TRL 6--7 de estas arquitecturas en mantenimiento predictivo industrial --- parcialmente resuelto mediante la revisión sistemática de \citet{henaovilla2026}, con la salvedad de que la equivalencia entre su clasificación de rigor de validación y la escala TRL es una interpretación propia pendiente de verificar contra la metodología completa del artículo.
\end{enumerate}
